...
class DataLoaderClear:
    ...
    def __get_list__(self):
        ...
        print('read train txt(%s: %d)' % (self.style_B, len(self.list_B_train)))
        self.list_B_val = self.__get_lines_from_txt(self.style_B, '_val')
        print('read val txt(%s: %d)' % (self.style_B, len(self.list_B_val)))

        random.shuffle(self.list_B_train)
        self.list_B_train = self.list_B_train[: self.num_per_epoch]
        print('data train per epoch(%s: %d)' % (self.style_B, len(self.list_B_train)))
    ...
    def load_batch_train(self, batch_size=1, is_testing=False):
        ...
        return self.load_batch(path_B, batch_size, is_testing)
    ...
    def load_batch_val(self, batch_size=1, is_testing=True, random=True):
        ...
        for p_B in path_B:
            pool.apply_async(self.__read_img_item__, args=(p_B, list_dict, is_testing))
        pool.close()
        pool.join()
    ...
    def __read_img_item__(self, p_B, list_dict, is_testing=False):
        '''
    ...
    def load_batch(self, path_B, batch_size, is_testing):
...
if __name__ == '__main__':
    ...
    dl = DataLoaderClear(conf)

    save_path = r'E:\tmp'


    for i in range(100):
        items = dl.load_batch_val(batch_size=1, is_testing=True, random=True)

        imgs = []

        img_B = items['imgs_B'][0]
        imgs.append(img_B)
        imgs_B_det = items['imgs_B_det'][0]

        for ind in range(len(imgs_B_det)):
            img_det = imgs_B_det[ind]
            imgs.append(img_det)

        imgs = np.uint8((np.float32(imgs) + 1) * 127.5)
        img = cv2.hconcat(imgs, )
        img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
        cv2.imwrite(os.path.join(save_path, '%d.jpg' % i), img)